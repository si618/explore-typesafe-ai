...
| Path | Content | [FHIR](vocabulary.md#fhir) |
| --- | --- | --- |
| [`data/synthea-r4/`](https://github.com/si618/explore-typesafe-ai/blob/main/data/synthea-r4) | Complete [Synthea](vocabulary.md#synthea) bundles for the original 100, gzipped, including claims | R4 4.0.1 + [US Core](vocabulary.md#us-core), as exported |
...
| [`data/scenarios/`](https://github.com/si618/explore-typesafe-ai/blob/main/data/scenarios) | Case definitions and [**reference labels**](vocabulary.md#reference-label), each set committed before its first model run | – |
...
[Jev](vocabulary.md#jev) never sees raw FHIR. Code builds a small, named JSON `state` for each question set, with only the fields that decision needs. The [jaggedness notes](https://docs.typesafe.ai/model-jaggedness/jev-1.13) warn that irrelevant state costs accuracy.
