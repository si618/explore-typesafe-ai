...
This repo tests [TypeSafe](https://docs.typesafe.ai)'s [**System One**](vocabulary.md#system-one) model, [**Jev**](vocabulary.md#jev), on five hospital tasks over **1,000 synthetic [FHIR](vocabulary.md#fhir) patients**. Claude models act as author, escalation reviewer and LLM baseline. Jev doesn't generate text: it returns **typed answers with calibrated probabilities** ([Choice](vocabulary.md#choice), [Score](vocabulary.md#score) and [Noul](vocabulary.md#noul)) that code can branch on. The design question throughout is *which part of a clinical decision is a fast semantic judgment, and which part belongs in code or in a slower reasoning model?*
...
| Scenario | [Primitives](vocabulary.md#primitive) → [task categories](vocabulary.md#task-categories) | Result |
| --- | --- | --- |
| [1. Ward deterioration](s1-ward.md) | Noul → detection · Score → scoring/ranking · Choice → classification | [NEWS2](vocabulary.md#news2) alone [under-triaged](vocabulary.md#under-triage) **10/20** hand and **22/40** test patients; with Jev reading the note, **1/20** and **8/40**. |
| [2. Discharge med reconciliation](s2-discharge.md) | Choice [fan-out](vocabulary.md#fan-out) → structured extraction · Noul → verification · Score → scoring | **98%** hand / **98%** test medication statuses extracted correctly; allergy check 100% on test. Whole-regimen [interaction](vocabulary.md#interaction) checks are weak; [decomposing them](generated.md#discharge-decomposing-the-weak-checks) helps but doesn't fix them. |
| [3. Post-discharge inbox](s3-inbox.md) | Choice → routing · Score → urgency · Noul → detection | The [confidence gate](vocabulary.md#confidence-gate) auto-dispatched **7/20** hand and **16/40** test messages, **all correctly**; the rest went to review. None of the 5 prompt-injection messages was routed where the injection asked. |
...
| [5. ML features](s5-features.md) | Score/Noul/Choice → feature extraction | Jev features from one note match structured data for predicting acute care (AUROC 0.64 vs 0.64); combined 0.65. [Synthea](vocabulary.md#synthea) caps what any feature can show. |
| [System Two review](system-two.md) | [Confidence](vocabulary.md#confidence) → escalation | 65 of 403 hand-case judgments (16%) escalated to a blinded Claude Sonnet 5 reviewer. |
...
| [Independent labels](independent-labels.md) | [Reference labels](vocabulary.md#reference-label) checked by another model family | A blind Codex pass differs from the Claude reference on **34/302** sampled judgments (κ 0.73 Noul, 0.94 Choice, 0.80 Score), mostly in discharge [reconciliation](vocabulary.md#medication-reconciliation); in 24 of them Jev gave the independent answer. |

**Cost and speed:** 24,557 typed Jev judgments in 4,722 requests, **[p50](vocabulary.md#p50) 317 ms** per request (1 to 71 questions each), **$0.26** in total. See [models, timing & tokens](performance.md).
